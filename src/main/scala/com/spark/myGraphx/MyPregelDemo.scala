...
object MyPregelDemo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val vertex = sc.parallelize(Array(
      (3L,List("3")),(7L,List("7")),(5L,List("5")),(2L,List("2")), (6L,List("6")), (8L,List("8"))
    ))

    // 创建边
    val edge = sc.parallelize(Array(
      Edge(3L, 7L, List("a")),Edge(2L, 5L, List("c")), Edge(5L, 7L, List("d")), Edge(2L, 7L, List("b")),
      Edge(3L, 5L, List("b")),Edge(6L, 5L, List("b")), Edge(8L, 6L,List("b")), Edge(8L, 2L, List("d")), Edge(8L, 3L, List("d"))
    ))
...
//      triple.srcAttr.head.toString.contains(start) || triple.srcAttr.head.toString.contains("3") || triple.srcAttr.head.toString.contains("6")
//      ||
      if(triple.srcAttr.head.toString.contains("8")){
