...
object MyDbscan {
  ...
  // 深度优先遍历树
  def mergeCluster(m: mMap[(Double, Double), List[(Double, Double)]], p: (Double, Double), minPts: Int): List[(Double, Double)] = {
    m.get(p) match{
      case Some(x) if x.length > minPts =>
        ...
        // 递归方法深度优先遍历树
        x.flatMap{y =>
          List(y) ++ mergeCluster(m, y, minPts)
        } ++ List(p)
      case _ => List((-1.0,-1.0))
    }
  }
  ...
  def dbscan(l: List[(Double, Double)], eps: Double, minPts: Int): List[List[(Double, Double)]] = {
    ...
    l.map{x =>
      if(v.getOrElse(x, List()).size > minPts)
        mergeCluster(v, x, minPts).filter(_ != (-1.0,-1.0)).distinct
      else
        List()
    }.filter(_.nonEmpty)
  }
